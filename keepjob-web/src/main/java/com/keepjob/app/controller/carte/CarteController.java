package com.keepjob.app.controller.carte;
...
public class CarteController {
	...
	public String toCarteView(HttpSession session, HttpServletRequest request){
		request.setAttribute("types",carteHandler.findCarteTypes());
		return "app/carte/carteView";
	}
	...
	@RequestMapping(value = "/addCarte.html", method = RequestMethod.GET)
	public String toCarteAddView(HttpSession session, HttpServletRequest request){
		request.setAttribute("types",carteHandler.findCarteTypes());
		return "app/carte/carteAdd";
	}
	
	@RequestMapping(value = "/editCarte.html", method = RequestMethod.GET)
	public String toCarteEditView(HttpSession session, HttpServletRequest request, @ModelAttribute("parameter") Carte parameter){
		request.setAttribute("types",carteHandler.findCarteTypes());
		request.setAttribute("result", parameter);
		return "app/carte/carteEdit";
	}
	...
	@ResponseBody
	public String startCartes(HttpSession session, HttpServletRequest request, @RequestParam(value = "ids", required = false) List<Integer> ids){
		try {
			User user = (User) session.getAttribute(Constant.USER_KEY);
	...
	@ResponseBody
	public String stopCartes(HttpSession session, HttpServletRequest request, @RequestParam(value = "ids", required = false) List<Integer> ids){
		try {
			User user = (User) session.getAttribute(Constant.USER_KEY);
	...
	@ResponseBody
	public String removeCartes(HttpSession session, HttpServletRequest request, @RequestParam(value = "ids", required = false) List<Integer> ids){
		try {
			User user = (User) session.getAttribute(Constant.USER_KEY);
