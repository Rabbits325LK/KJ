					...
					<table class="table table-hover table-condensed">
						<tr>
							...
							<td style=""><input id="name" name="name" type="text"
								placeholder="请输入店铺名称"
								class="easyui-validatebox input-sm form-control" value="${result.name }"
								data-options="required:true">
								<input type="hidden" name="id" value="${result.id }"/>
								<input type="hidden" name="uniqueCode" value="${result.uniqueCode }"/>
								</td>
							...
							<td><input type="text" id="principal" name="principal"
								...
								data-options="required:true" value="${result.principal }"></td>
						</tr>
						<tr>
							<td style="text-align: right;">地址:</td>
							<td><input type="text" name="address" id="address"
								placeholder="" class="easyui-validatebox input-sm form-control"
								data-options="" value="${result.address }"></td>
							<td style="text-align: right;">电话:</td>
							<td><input type="text" name="phone" id="phone" value="${result.phone }"
								placeholder="" class="easyui-validatebox input-sm form-control"
								data-options=""></td>
						</tr>
						<tr>
							...
							<td><select id="type" name="type"
								class="input-sm form-control">
									...
									<c:forEach items="${types }" var="t">
										<option value="${t.uniqueCode }"<c:if test="${t.uniqueCode == result.type }">selected="selected"</c:if>>${t.name }</option>
									</c:forEach>
							</select></td>
						...
						<tr>
							...
							<td colspan="3"><textarea id="summary" name="summary"
									class="form-control" rows="3" cols="65">${result.summary }</textarea></td>
						</tr>
						<tr>
