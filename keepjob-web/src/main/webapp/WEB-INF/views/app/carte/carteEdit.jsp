			...
			<div id="tabs" class="easyui-tabs" style="width: 650px;">
				<div id="basicDiv" title="基本信息" data-options="selected:true"
					...
					<table class="table table-hover table-condensed"
						style="width: 100%;">
						<tr>
							<td colspan="2" style="width: 50%;"><img id="logoImage"
								alt="" src="${result.carteLogo }"
								...
								<table>
									<tr>
										...
										<td style=""><input id="carteName" name="carteName"
											...
											value="${result.carteName }" data-options="required:true">
										...
										<td><select id="carteType" name="carteType"
											class="input-sm form-control">
												...
												<c:forEach items="${types }" var="t">
													<option value="${t.uniqueCode }" <c:if test="${result.carteType == t.uniqueCode }">selected="selected"</c:if>>${t.name }</option>
												</c:forEach>
										</select></td>
									</tr>
									<tr>
										...
										<td><select id="status" name="status"
											class="input-sm form-control">
												<option value="" >请选择</option>
												<option value="1" <c:if test="${result.status == 1 }">selected="selected"</c:if>>发布</option>
												<option value="0" <c:if test="${result.status == 0 }">selected="selected"</c:if>>暂存</option>
										</select></td>
									</tr>
									<tr>
										<td colspan="2"><input type="file" id="logoFile"
											name="logoFile"><br /> <input type="button"
											value="上传图片" onclick="ajaxFileUpload()" /><br />
											<p id="resultContext"></p> <input type="hidden"
											name="carteLogo" id="carteLogo" value="${result.carteLogo }" /></td>
									</tr>
								</table>
							</td>
						</tr>
						...
						<tr>
							<td colspan="4"><textarea id="carteSummary"
									name="carteSummary" class="form-control" rows="3"
									style="margin-left: 5%; width: 90%">${result.carteSummary }</textarea></td>
						</tr>
						<tr>
							<td colspan="4" style="text-align: center">菜谱:</td>
						</tr>
						<tr>
							<td colspan="4"><textarea id="carteContent"
									name="carteContent" class="form-control" rows="6"
									style="margin-left: 5%; width: 90%">${result.carteContent }</textarea></td>
						</tr>
						<tr>
